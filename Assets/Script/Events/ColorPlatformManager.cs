...
public class ColorPlatformManager : MonoBehaviour
{
    ...
    private float CalculateHeight(Color objectColor, Color referenceColor)
    {
        ...
        if (colorDistance >= colorThresholdMax) {
            //Debug.Log("too difference"); // Debug message
    ...
    private IEnumerator AdjustPlatform(Transform platform, float targetHeight)
    {
        ...
        {
            platform.localPosition = Vector3.Lerp(startPosition, targetPosition, elapsedTime);
            elapsedTime += Time.deltaTime * moveSpeed;
            yield return null;
        }

        platform.localPosition = targetPosition; // Ensure exact position after animation
        Debug.Log($"{platform.name} final height: {platform.localPosition.y}");
    }
}
